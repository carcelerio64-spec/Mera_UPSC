...
def _is_completed(s,user_id:int,exam:str,paper:str,subject:str,topic:str)->bool:
    return s.query(CompletedTopic).filter(CompletedTopic.user_id==user_id,CompletedTopic.exam==exam.lower(),CompletedTopic.paper==paper,CompletedTopic.subject==subject,CompletedTopic.topic==topic).first() is not None

def _target(exam:str)->int:return PRELIMS_TARGET if exam.lower()=='prelims' else MAINS_TARGET
...
def build_topic_router(current_user):
    ...
    def set_topic_completion(x:TopicCompletionIn,u=Depends(current_user)):
        exam=x.exam.lower()
        if not _find_topic(exam,x.paper,x.subject,x.topic):raise HTTPException(404,'Topic is not in the loaded UPSC syllabus')
        s=SessionStudy()
        try:
            ...
            s.commit()
            count=s.query(QuestionBank).filter(QuestionBank.exam==exam,QuestionBank.paper==x.paper,QuestionBank.subject==x.subject,QuestionBank.topic==x.topic).count();target=_target(exam)
    ...
    def completed_topic_question_sections(exam:str,u=Depends(current_user)):
        ...
        try:
            ...
            for c in completed:
                count=s.query(QuestionBank).filter(QuestionBank.exam==exam,QuestionBank.paper==c.paper,QuestionBank.subject==c.subject,QuestionBank.topic==c.topic).count()
                sections.append({'section_key':f'{exam}|{c.paper}|{c.subject}|{c.topic}','exam':exam,'paper':c.paper,'subject':c.subject,'topic':c.topic,'completed_at':c.completed_at.isoformat(),'question_target':target,'saved_questions':count,'generation_needed':max(0,target-count),'ready':count>=target})
    ...
    def topic_study(exam:str,paper:str,subject:str,topic:str,u=Depends(current_user)):
        exam=exam.lower()
        if not _find_topic(exam,paper,subject,topic):raise HTTPException(404,'Topic is not in the loaded UPSC syllabus')
        s=SessionStudy()
        try:
            ...
            question_count=s.query(QuestionBank).filter(QuestionBank.exam==exam,QuestionBank.paper==paper,QuestionBank.subject==subject,QuestionBank.topic==topic).count();completed=_is_completed(s,u.id,exam,paper,subject,topic);target=_target(exam)
    ...
    def questions_by_topic(exam:str,paper:str,subject:str,topic:str,limit:int=100,u=Depends(current_user)):
        ...
        s=SessionStudy()
        try:
            if not _is_completed(s,u.id,exam,paper,subject,topic):raise HTTPException(403,'Complete this topic first. Its separate question section unlocks after completion.')
            rows=s.query(QuestionBank).filter(QuestionBank.exam==exam,QuestionBank.paper==paper,QuestionBank.subject==subject,QuestionBank.topic==topic).order_by(QuestionBank.id.asc()).limit(max(1,min(limit,_target(exam)))).all();details=question_detail_map([r.id for r in rows])
            questions=[]
            for r in rows:
                d=details.get(r.id,{})
                questions.append({'id':r.id,'subtopic':r.subtopic,'question':r.question,'difficulty':r.difficulty,'source':r.source,'question_type':d.get('question_type','mcq' if exam=='prelims' else 'mains'),'options':d.get('options',[]),'marks':d.get('marks',0),'word_limit':d.get('word_limit',0)})
