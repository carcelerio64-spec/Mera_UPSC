...
def build_mains_marking_router(current_user):
    router=APIRouter()
    @router.put('/mains/answers/{submission_id}/evaluation')
    def mark(submission_id:int,x:MarkingIn,u=Depends(current_user)):
        s=SessionStudy()
        try:
            sub=s.get(MainsAnswerSubmission,submission_id)
            if not sub or sub.user_id!=u.id:raise HTTPException(404,'Submission not found')
            q=s.get(QuestionBank,sub.question_id);d=s.query(QuestionDetail).filter(QuestionDetail.question_id==sub.question_id).first()
            ...
            if not row:row=MainsAnswerEvaluation(submission_id=sub.id,question_id=sub.question_id,user_id=u.id,max_marks=max_marks);s.add(row)
            ...
            return {'submission_id':sub.id,'question_id':q.id,'marks_awarded':row.marks_awarded,'max_marks':max_marks,'label':'UPSC-pattern AI Evaluation','official_upsc_marks':False,'strengths':row.strengths,'improvements':row.improvements,'model_framework':row.model_framework}
        finally:s.close()
