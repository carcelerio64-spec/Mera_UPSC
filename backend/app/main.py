...
class Topic(Base):
    ...
    id:Mapped[int]=mapped_column(primary_key=True)
    exam:Mapped[str]=mapped_column(String(20)); paper:Mapped[str]=mapped_column(String(60)); subject:Mapped[str]=mapped_column(String(120)); title_hi:Mapped[str]=mapped_column(String(200)); title_en:Mapped[str]=mapped_column(String(200)); sort_order:Mapped[int]=mapped_column(Integer,default=0)
class Progress(Base):
...
def token_for(uid:int):
    return jwt.encode({'sub':str(uid),'exp':datetime.now(timezone.utc)+timedelta(minutes=ACCESS_MINUTES)},SECRET_KEY,algorithm=ALGORITHM)
...
class Signup(BaseModel):
    name:str; email:EmailStr; password:str; target_year:int=2027; language:str='hi'
class UserOut(BaseModel):
    id:int; name:str; email:str; target_year:int; language:str
...
def signup(x:Signup,s:Session=Depends(db)):
    ...
    u=User(name=x.name,email=x.email,password_hash=pwd.hash(x.password),target_year=x.target_year,language=x.language); s.add(u); s.commit(); s.refresh(u)
