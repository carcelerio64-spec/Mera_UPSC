...
app=FastAPI(title='UPSC Prep API', version='1.0.0')
...
def current_user(tok:str=Depends(oauth2), s:Session=Depends(db)):
    try:
        payload=jwt.decode(tok, SECRET_KEY, algorithms=[ALGORITHM]); uid=int(payload['sub'])
...
def seed():
    ...
    if s.query(Topic).count()==0:
        data=[
            ...
            ('prelims','GS Paper-I','इतिहास','प्राचीन भारत','Ancient India'),
            ...
            ('mains','GS-I','भारतीय समाज','भारतीय समाज की विशेषताएँ','Salient Features of Indian Society'),
...
def signup(x:Signup,s:Session=Depends(db)):
    ...
    u=User(name=x.name,email=x.email,password_hash=pwd.hash(x.password),target_year=x.target_year,language=x.language)
...
def save_progress(x:ProgressIn,u:User=Depends(current_user),s:Session=Depends(db)):
    ...
    p.status=x.status; p.percent=max(0,min(100,x.percent)); p.updated_at=datetime.now(timezone.utc)
...
def submit_mock(x:MockIn,u:User=Depends(current_user),s:Session=Depends(db)):
    ...
    r=MockResult(user_id=u.id,paper=x.paper,total_questions=x.total_questions,correct=x.correct,wrong=x.wrong,unattempted=x.unattempted,marks_per_question=x.marks_per_question,negative_fraction=x.negative_fraction,score=round(score,2),duration_seconds=x.duration_seconds)
...
def dashboard(u:User=Depends(current_user),s:Session=Depends(db)):
    ps=s.query(Progress).filter(Progress.user_id==u.id).all(); done=sum(1 for p in ps if p.status=='completed')
    total=s.query(Topic).count(); overall=round(done/max(1,total)*100)
    return {'name':u.name,'target_year':u.target_year,'overall_progress':overall,'revision_due':sum(1 for p in ps if p.status=='revision_due'),'today_topics':3,'continue_learning':'मौलिक अधिकार'}
