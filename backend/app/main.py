...
ALGORITHM='HS256'; ACCESS_MINUTES=60*24*7
...
class User(Base):
    __tablename__='users'
    id:Mapped[int]=mapped_column(primary_key=True);name:Mapped[str]=mapped_column(String(120));email:Mapped[str]=mapped_column(String(200),unique=True,index=True);password_hash:Mapped[str]=mapped_column(String(255));target_year:Mapped[int]=mapped_column(Integer,default=2027);language:Mapped[str]=mapped_column(String(20),default='hi')
class Topic(Base):
    __tablename__='topics'
    id:Mapped[int]=mapped_column(primary_key=True);exam:Mapped[str]=mapped_column(String(20));paper:Mapped[str]=mapped_column(String(60));subject:Mapped[str]=mapped_column(String(120));title_hi:Mapped[str]=mapped_column(String(200));title_en:Mapped[str]=mapped_column(String(200));sort_order:Mapped[int]=mapped_column(Integer,default=0)
class Progress(Base):
    __tablename__='progress'
    id:Mapped[int]=mapped_column(primary_key=True);user_id:Mapped[int]=mapped_column(ForeignKey('users.id'),index=True);topic_id:Mapped[int]=mapped_column(ForeignKey('topics.id'),index=True);status:Mapped[str]=mapped_column(String(30),default='not_started');percent:Mapped[int]=mapped_column(Integer,default=0);updated_at:Mapped[datetime]=mapped_column(DateTime,default=lambda:datetime.now(timezone.utc))
class MockResult(Base):
    __tablename__='mock_results'
    id:Mapped[int]=mapped_column(primary_key=True);user_id:Mapped[int]=mapped_column(ForeignKey('users.id'),index=True);paper:Mapped[str]=mapped_column(String(60));total_questions:Mapped[int]=mapped_column(Integer);correct:Mapped[int]=mapped_column(Integer);wrong:Mapped[int]=mapped_column(Integer);unattempted:Mapped[int]=mapped_column(Integer);marks_per_question:Mapped[float]=mapped_column(Float);negative_fraction:Mapped[float]=mapped_column(Float,default=1/3);score:Mapped[float]=mapped_column(Float);duration_seconds:Mapped[int]=mapped_column(Integer);submitted_at:Mapped[datetime]=mapped_column(DateTime,default=lambda:datetime.now(timezone.utc),index=True)
...
class ProgressIn(BaseModel):topic_id:int;status:str;percent:int
class MockIn(BaseModel):paper:str='GS Paper-I';total_questions:int=100;correct:int;wrong:int;unattempted:int;marks_per_question:float=2.0;duration_seconds:int
@app.on_event('startup')
def seed():
    s=SessionLocal()
    if s.query(Topic).count()==0:
        data=[('prelims','GS Paper-I','राजव्यवस्था','संविधान की मूल बातें','Constitution Basics'),('prelims','GS Paper-I','राजव्यवस्था','मौलिक अधिकार','Fundamental Rights'),('prelims','GS Paper-I','इतिहास एवं संस्कृति','प्राचीन भारत','Ancient India'),('prelims','GS Paper-I','भूगोल','भारतीय भौतिक भूगोल','Physical Geography of India'),('prelims','GS Paper-I','अर्थव्यवस्था','मुद्रास्फीति','Inflation'),('prelims','GS Paper-I','पर्यावरण','जैव विविधता','Biodiversity'),('prelims','GS Paper-I','विज्ञान एवं प्रौद्योगिकी','अंतरिक्ष एवं प्रौद्योगिकी','Space and Technology'),('prelims','GS Paper-I','कृषि','भारतीय कृषि','Indian Agriculture'),('prelims','GS Paper-I','सामाजिक मुद्दे','स्वास्थ्य एवं शिक्षा','Health and Education'),('prelims','GS Paper-I','अंतरराष्ट्रीय संबंध','अंतरराष्ट्रीय संगठन','International Organisations'),('prelims','CSAT','CSAT','रीडिंग कॉम्प्रिहेंशन','Reading Comprehension'),('mains','GS-I','इतिहास एवं संस्कृति','भारतीय संस्कृति','Indian Culture'),('mains','GS-I','भूगोल','भारतीय एवं विश्व भूगोल','Indian and World Geography'),('mains','GS-I','सामाजिक मुद्दे','भारतीय समाज की विशेषताएँ','Salient Features of Indian Society'),('mains','GS-II','राजव्यवस्था','संघवाद','Federalism'),('mains','GS-II','अंतरराष्ट्रीय संबंध','भारत और विश्व','India and the World'),('mains','GS-III','अर्थव्यवस्था','समावेशी विकास','Inclusive Growth'),('mains','GS-III','कृषि','कृषि एवं खाद्य प्रबंधन','Agriculture and Food Management'),('mains','GS-III','पर्यावरण','पर्यावरण एवं जलवायु परिवर्तन','Environment and Climate Change'),('mains','GS-III','विज्ञान एवं प्रौद्योगिकी','विज्ञान एवं प्रौद्योगिकी','Science and Technology'),('mains','GS-IV','नीतिशास्त्र','ईमानदारी और निष्पक्षता','Integrity and Impartiality'),('mains','Essay','निबंध','समसामयिक एवं दार्शनिक विषय','Contemporary and Philosophical Themes')]
        for i,(e,p,sub,hi,en) in enumerate(data):s.add(Topic(exam=e,paper=p,subject=sub,title_hi=hi,title_en=en,sort_order=i))
        s.commit()
    s.close()
@app.get('/health')
...
def topics(exam:Optional[str]=None,u:User=Depends(current_user),s:Session=Depends(db)):
    q=s.query(Topic)
    if exam:q=q.filter(Topic.exam==exam)
    rows=q.order_by(Topic.sort_order).all();pmap={p.topic_id:p for p in s.query(Progress).filter(Progress.user_id==u.id).all()}
...
@app.get('/exam-history/prelims')
def prelims_history(limit:int=100,u:User=Depends(current_user),s:Session=Depends(db)):
    rows=s.query(MockResult).filter(MockResult.user_id==u.id).order_by(MockResult.submitted_at.desc(),MockResult.id.desc()).limit(max(1,min(limit,500))).all()
    return [{'id':r.id,'paper':r.paper,'total_questions':r.total_questions,'correct':r.correct,'wrong':r.wrong,'unattempted':r.unattempted,'marks_per_question':r.marks_per_question,'negative_fraction':r.negative_fraction,'score':r.score,'max_marks':round(r.total_questions*r.marks_per_question,2),'duration_seconds':r.duration_seconds,'submitted_at':r.submitted_at.isoformat() if r.submitted_at else None} for r in rows]
