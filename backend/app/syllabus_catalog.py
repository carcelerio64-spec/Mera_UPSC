"""Structured UPSC CSE syllabus catalog.

This module stores a stable, app-friendly hierarchy. Text is intentionally concise/paraphrased
for navigation; source of truth remains the UPSC examination notification.
"""
...
OPTIONAL_SUBJECTS = [
    ...
    'Philosophy','Physics','Political Science & International Relations','Psychology','Public Administration',
    'Sociology','Statistics','Zoology','Assamese Literature','Bengali Literature','Bodo Literature',
    'Dogri Literature','Gujarati Literature','Hindi Literature','Kannada Literature','Kashmiri Literature',
    'Konkani Literature','Maithili Literature','Malayalam Literature','Manipuri Literature','Marathi Literature',
    'Nepali Literature','Odia Literature','Punjabi Literature','Sanskrit Literature','Santhali Literature',
    'Sindhi Literature','Tamil Literature','Telugu Literature','Urdu Literature','English Literature'
]
    ...
    return [
        {
            'subject': subject,
            'papers': [
                {'paper':'Paper-I','topics':[]},
                {'paper':'Paper-II','topics':[]},
            ],
            'detailed_topics_loaded': False,
        }
        for subject in OPTIONAL_SUBJECTS
    ]


def syllabus_for(exam: str):
    key=(exam or '').lower()
    if key=='prelims': return PRELIMS
    if key=='mains': return MAINS
    if key=='optional': return optional_catalog()
    return {'prelims':PRELIMS,'mains':MAINS,'optional':optional_catalog()}
