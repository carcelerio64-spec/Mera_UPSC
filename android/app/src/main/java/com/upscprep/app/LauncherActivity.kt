    ...
    Scaffold(bottomBar = {
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when (page) {
                ...
                "topic_study" -> studyTarget?.let { target -> if (demo) DemoTopicStudyScreen(target, { page = "demo_questions" }) { page = if (target.exam == "prelims") "prelims_syllabus" else "mains_syllabus" } else TopicStudyV2Screen(token, target) { page = if (target.exam == "prelims") "prelims_syllabus" else "mains_syllabus" } }
                "demo_questions" -> studyTarget?.let { target -> DemoQuestionScreen(target, { q -> answerQuestion = q; page = "demo_answer" }) { page = "topic_study" } }
                "demo_completed" -> {
                ...
                "topic_questions" -> selectedSection?.let { section -> TopicQuestionListScreen(token, section, { page = "completed_practice" }, { q -> answerQuestion = q; page = "mains_answer" }) }
                ...
                "demo_answer" -> DemoFeatureScreen("MAINS ANSWER WRITING", listOf("Question: ${answerQuestion?.question ?: "Test question"}", "10/15 Marks • 150/250 Words", "Typed draft", "PDF / Photo / Camera Upload", "UPSC-pattern AI Evaluation", "Model Answer handwritten upload के बाद unlock")) { page = "mains" }
...
private fun DemoHomeScreen(onOpen: (String) -> Unit, onLogout: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        listOf("PRELIMS" to "prelims", "MAINS" to "mains", "OPTIONAL" to "optional", "AI TEACHER" to "demo_ai", "CURRENT AFFAIRS" to "demo_current", "CLASS NOTES" to "demo_notes", "PYQ" to "demo_pyq", "REVISION" to "demo_revision").forEach { (title, route) ->
            ElevatedCard(onClick = { onOpen(route) }, modifier = Modifier.fillMaxWidth().padding(vertical = 5.dp)) { Text(title, Modifier.padding(18.dp)) }
        }
...
private fun DemoFeatureScreen(title: String, items: List<String>, onBack: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        items.forEach { item -> ElevatedCard(Modifier.fillMaxWidth().padding(vertical = 5.dp)) { Text(item, Modifier.padding(18.dp)) } }
    }
}
...
private fun DemoCompletedScreen(exam: String, targets: List<StudyTarget>, open: (StudyTarget) -> Unit, back: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        targets.forEach { target -> Card(onClick = { open(target) }, modifier = Modifier.fillMaxWidth().padding(vertical = 6.dp)) { Column(Modifier.padding(18.dp)) { Text(target.paper); Text(target.subject, style = MaterialTheme.typography.titleMedium); Text(target.topic) } } }
    }
}
