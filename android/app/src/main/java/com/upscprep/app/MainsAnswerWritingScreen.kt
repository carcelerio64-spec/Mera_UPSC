...
fun MainsAnswerWritingScreen(context:Context,token:String,question:TopicQuestionDto,onBack:()->Unit){
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp)){
        ...
        Button(enabled=!busy&&answer.isNotBlank(),onClick={scope.launch{
            busy=true;message="";try{submission=ApiClient.api.submitTypedAnswer(ApiClient.bearer(token),TypedAnswerRequest(question.id,answer));message="Answer submit हो गया"}catch(_:Exception){message="Answer submit नहीं हुआ"}finally{busy=false}
        ...
        selectedFile?.let{f->
            ...
            Button(enabled=!busy,onClick={scope.launch{
                busy=true;message="";try{
                    ...
                    submission=ApiClient.api.uploadMainsAnswer(ApiClient.bearer(token),qid,part);message="Handwritten answer upload हो गया"
